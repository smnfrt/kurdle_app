...
class WordService {
  ...
  Future<void> init() async {
    final rawAnswers = await _readFile('assets/answers.txt');
    final rawGuesses = await _readFile('assets/allowed_guesses.txt');
    _answers = rawAnswers.map((w) => w.trim().toUpperCase()).where((w) => w.isNotEmpty).toList();
    _guesses = rawGuesses.map((w) => w.trim().toUpperCase()).where((w) => w.isNotEmpty).toList();
  }
  ...
  bool isValidGuess(String guess) {
    return _combined.contains(guess.toUpperCase());
  }
