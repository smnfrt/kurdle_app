...
// XP → seviye eşikleri
const _levelThresholds = [0, 200, 500, 1000, 2000, 3500, 5500, 8000, 11000, 15000];

String _levelTitle(int level) {
  const titles = [
    'Yeni Başlayan', 'Çırak', 'Öğrenci', 'Usta Aday', 'Usta',
    'İleri Usta', 'Uzman', 'Şampiyon', 'Efsane', 'Peyvok Ustası',
  ];
...
double _xpProgress(int xp, int level) {
  final lo = level <= _levelThresholds.length ? _levelThresholds[level - 1] : _levelThresholds.last;
  final hi = level < _levelThresholds.length  ? _levelThresholds[level]     : _levelThresholds.last + 5000;
  if (hi <= lo) return 1.0;
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _load() async {
    ...
    if (mounted) {
      setState(() {
        _profile = profile;
        _nameCtrl.text = profile?.displayName ?? AuthService.instance.currentUser?.displayName ?? '';
        _loading = false;
      });
    }
  }
          ...
          if (_editing)
            Row(
              children: [
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      border: OutlineInputBorder(
                        ...
                        borderSide: BorderSide(color: _kPrimary.withOpacity(0.5)),
                      ),
                ...
                GestureDetector(
                  onTap: () => setState(() { _editing = false; _nameCtrl.text = _profile?.displayName ?? ''; }),
                  child: Container(
                    padding: const EdgeInsets.all(10),
                    decoration: BoxDecoration(
                      color: Colors.white.withOpacity(0.07),
  ...
  Widget _buildStatsCard() {
    ...
    final items = [
      (icon: Icons.sports_esports_rounded, color: const Color(0xFF64B5F6), label: 'Oyun',      value: '${s.played}'),
      (icon: Icons.emoji_events_rounded,   color: _kPrimary,               label: 'Kazanma',   value: '%$winRate'),
      (icon: Icons.local_fire_department,  color: const Color(0xFFFF7043), label: 'Seri',      value: '${s.streak}'),
      (icon: Icons.star_rounded,           color: _kGold,                  label: 'En Yüksek', value: '${s.highScore}'),
    ];
